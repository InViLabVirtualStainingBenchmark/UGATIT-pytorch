#!/bin/bash
#SBATCH --job-name=ugatit_validate_BCI_i200
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH -p ampere_gpu
#SBATCH --gres=gpu:1
#SBATCH -o /data/antwerpen/212/vsc21212/projects/ugatit/logs/train_validate_BCI_i200.%j.out
#SBATCH -e /data/antwerpen/212/vsc21212/projects/ugatit/logs/train_validate_BCI_i200.%j.err

# train_validate_BCI_i200.sh
# Runs 200 iterations of UGATIT training on BCI as a cluster confirmation gate.
...
#        ls $VSC_DATA/projects/ugatit/outputs/validate/BCI_validate_i200/model/
#   4. GPU log has entries:
#        tail -5 $VSC_DATA/projects/ugatit/logs/gpu_train_validate_BCI_i200.csv
#   5. Record time-per-1000-iterations to estimate full-run wall time:
#        Full run per part: 500,000 iterations. Estimate: (sec/1000iter * 500 * 1.20) / 3600 hr
...
nvidia-smi --query-gpu=timestamp,utilization.gpu,memory.used,memory.total \
           --format=csv -l 5 \
    > "$VSC_DATA/projects/ugatit/logs/gpu_train_validate_BCI_i200.csv" & GPU_LOG_PID=$!
...
echo ""
echo "=== GPU log tail ==="
tail -3 "$VSC_DATA/projects/ugatit/logs/gpu_train_validate_BCI_i200.csv"

echo ""
echo "Validation training complete. Review the output above before submitting full runs."
echo "Record time-per-1000-iterations to estimate wall time for train_full_BCI_i1M_part1.sh."
